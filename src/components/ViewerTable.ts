...
const ViewerTable: m.ClosureComponent<ViewerTableAttrs> = ({
  attrs: {
    acts$,
    filterFromTable$,
    selectedActId$,
  ...
  function columnLabel(column: Column) {
    ...
    return m(
      ".ViewerTable-column-label",
      {
        class:
          sortColumn === column
            ? "ViewerTable-column-label-" + sortDir
            : undefined,
    ...
    view: () => {
      ...
      return m(
        ...
        m(
          ...
          {
            class: headerOpen
              ? "ViewerTable-header-open"
              : "ViewerTable-header-closed",
            style: {
              height:
                headerDom && `${headerOpen ? headerDom.scrollHeight : 0}px`,
              top: headerDom && `${headerOpen ? 0 : -headerDom.scrollHeight}px`,
            },
            oncreate: ({ dom }) => (headerDom = dom as HTMLElement),
          },
          m(
            ".ViewerTable-fake-row.ViewerTableRow",
            m(
              ".ViewerTableRow-left",
              m(
                "select.ViewerTableRow-type-selector",
                {
                  value: typeFilter$(),
                  onchange: (ev: InputEvent) =>
                    typeFilter$(
                      typeOptions[
                        (ev.target as HTMLSelectElement).selectedIndex
                      ],
                    ),
                },
            ...
            m(
              ...
              m(
                ".ViewerTableRow-name",
                m(
                  ".ViewerTable-name-filter-container",
                  m("input.ViewerTable-name-filter", {
                    value: nameFilter$(),
                    oninput: (ev: InputEvent) =>
                      nameFilter$((ev.target as HTMLInputElement).value),
          ...
          m(
            "select.ViewerTable-header-download-selector",
            {
              value: "⤓",
              onchange: (ev: InputEvent) => {
        ...
        m(
          ".ViewerTable-scroller",
          { oncreate: oncreateScroller },
          m(
            ".ViewerTable-acts",
            sortedActs$().map((act) =>
              m(ViewerTableRow, {
